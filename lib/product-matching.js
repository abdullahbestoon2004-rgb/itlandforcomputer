...
// Amazon ASINs (X00…/B0… followed by 7-8 alphanumerics) identify a marketplace
// listing, not a model. Several items carry one as their name or SKU, and
// treating it as a model code meant the correct art was rejected for "failing"
// to account for it — the Anker PowerConf could not match Anker_PowerConf.webp.
const ASIN = /^[xb]0[0-9a-z][0-9a-z]{6,8}$/;

// Console names belong here too: an accessory "for PS5, PS4, Nintendo Switch"
// names what it works with, not its own model, and reading those as model codes
// stopped Elgato's Chat Link Pro matching its own art.
const SPEC_TOKENS =
  ...
  /^(?:\d+(?:fps|hz|khz|w|k|v|a|mm|cm|ft|gb|tb|mb|mah|bit|p|gbps|mbps|dpi|in|ch|x)|usb\d*|type\d*|v\d(?:\d)?|\d+x\d+|4k|8k|2k|1080p|1440p|720p|3d|2in1|3in1|4in1|5in1|6in1|7in1|8in1|9in1|10in1|11in1|ps5|ps4|ps3|xbox\d*|switch2)$/;
...
const GENERIC_TOKENS = new Set([
  ...
  // Brand abbreviations that appear in filenames but not in product titles.
  // "OTN" is Onten's own prefix (Onten_OTN-UC620.webp), so requiring it to
  // appear in the listing would reject the brand's own art.
  'otn',
]);
...
export function findProductImage(item, { overrides = {}, images = [] } = {}) {
  ...
  const titleModelCodes = titleTokens.filter(t =>
    t.length >= 3 && /[a-z]/.test(t) && /\d/.test(t) &&
    !SPEC_TOKENS.test(t) && !ASIN.test(t));
  ...
  for (const file of images) {
    ...
    // Three characters, not five: "Cam" and "Link" are what separate the Cam
    // Link 4K from an "Elgato 4K S", and at a five-character threshold the two
    // shared only the resolution "4K" — enough to put the wrong capture card on
    // the listing.
    const seriesWords = modelTokens
      .flatMap(t => t.split(/[^a-z0-9]/))
      .filter(t => t.length >= 3 && /^[a-z]+$/.test(t) && !GENERIC_TOKENS.has(t)
        && !(imgBrand.startsWith(t) || t.startsWith(imgBrand)));
