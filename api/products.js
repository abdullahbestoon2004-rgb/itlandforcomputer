...
function findProductImage(item, overrides = {}) {
  ...
  for (const file of LOCAL_IMAGES) {
    ...
    if (productBrand) {
      ...
      if (!matchesWordExact(titleText, imgBrand) && !titleDense.includes(imgBrand)) {
        continue;
      }
    }
    ...
    // Exact full model name match in title / SKU (e.g. "mx master 3s", "brio 500", "otn-9591a", "stream deck mk2")
    ...
    for (const token of modelTokens) {
      ...
      for (const st of subTokens) {
        ...
        if (hasDigits && /[a-z]/.test(st)) {
          ...
          // Model number with 3+ digits (e.g. "120", "500", "4310")
          totalDistinctiveTokens++;
          if (matchesWordExact(titleText, st) || titleDense.includes(stDense)) {
